...
cdef class Class:
   ...
   #################################
    ...
    def get_tensor_transfer(self, z=0.):
        """
        Return h_+(k, z) and dh_+(k, z) from the standard CLASS tensor sector,
        which includes radiation (photon + neutrino) anisotropic stress damping.
        Requires 'modes' to include 't' in the CLASS parameter file.
        ...
        -------
        dict with keys:
            'k (h/Mpc)' : wavenumber array in h/Mpc
            'h_t'       : GW amplitude transfer function h(k, z)
            'dh_t'      : GW time-derivative transfer function hdot(k, z)
        """
        self.compute(["transfer"])

        if not self.pt.has_tensors:
            raise CosmoSevereError("Tensor modes are not active. Add 'modes = t' or 'modes = s,t'.")

        if not self.pt.has_source_gw_t:
            raise CosmoSevereError("Tensor h source not stored. This should be automatic when has_tensors is True.")

        cdef int index_md = self.pt.index_md_tensors
        cdef int index_ic = 0  # only one IC for tensors
        cdef int index_tau
        cdef int index_k
        cdef int i
        ...
        # Find the conformal time tau corresponding to redshift z
        if background_tau_of_z(&self.ba, z, &tau_req) == _FAILURE_:
            raise CosmoSevereError(self.ba.error_message)

        # Find the closest stored tau index
        index_tau = 0
        for i in range(self.pt.tau_size - 1):
            if self.pt.tau_sampling[i] <= tau_req:
                index_tau = i

        tp_size = self.pt.tp_size[index_md]
        cdef int idx_gw    = self.pt.index_tp_gw_t
        cdef int idx_gwdot = self.pt.index_tp_gwdot_t

        k_arr    = np.zeros(k_size, dtype=np.double)
        h_arr    = np.zeros(k_size, dtype=np.double)
        hdot_arr = np.zeros(k_size, dtype=np.double)

        for index_k in range(k_size):
            k_arr[index_k]    = self.pt.k[index_md][index_k] / self.ba.h
            h_arr[index_k]    = self.pt.sources[index_md][index_ic * tp_size + idx_gw][index_tau * k_size + index_k]
            hdot_arr[index_k] = self.pt.sources[index_md][index_ic * tp_size + idx_gwdot][index_tau * k_size + index_k]
